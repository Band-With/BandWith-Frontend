...
::v-deep .vm2_modal-dialog[data-v-11fe6c4e] {
  position: relative;
  max-width: 750px;
}

::v-deep .vm2_body[data-v-11fe6c4e] {
  overflow: visible;
}

::v-deep .vm2__hr[data-v-a2527186],
::v-deep .vm2__hr[data-v-4c1e6de4] {
  border-top: none;
}
/* ################################################## */
</style>
...
<template>
  <div id="body" class="d-flex">
    <div class="main d-flex" style="min-height: calc(100vh - 60px)">
      <!-- ################## invite modal ################## -->
      <vue-modal-2 name="modal-invite" @on-close="closeInvite">
        <template v-slot:header>
          <div class="d-flex justify-content-end p-3">
            <button class="btn" @click="closeInvite">
              <img
        ...
        <!-- modal body -->
        <div class="px-4" style="height: 250px;">
          <div
            class="d-flex flex-column align-items-center justify-content-center p-3"
          >
            <img src="@/assets/images/icon/email.png" style="width:50px; height:50px;" />
            <h4 data-view-component="true" class="mt-4 mb-5">
      ...
      <div
        id="band-info"
        class="d-flex flex-column position-fixed align-items-center py-5"
        :class="{ 'band-info-1': this.windowWidth < 1300 }"
      >
        <!-- aside -->
        <div class="d-flex flex-column mb-1">
          <span
            style="background-color: #EFEFEF; height: 290px; width: 290px; border-radius: 15px"
          ...
          <div
            class="mt-4 mb-3 d-flex flex-row justify-content-between align-items-end"
          >
            <span style="font-size: 24px; font-weight: bold">{{
              content.band.band_name
            }}</span>
            <span id="leave-button" @click="leave" v-if="memberOfBand"
              >탈퇴하기</span
            >
          </div>
          <span
            class="mb-4"
            style="font-size: 14px; font-weight: 300; color: #0095F6"
            >좋아요 {{ content.totalLikes }}개</span
          >
          <router-link
            v-if="memberOfBand"
            :to="{ name: 'bandMusic', params: { bandname: bandnameParam } }"
          >
            <button id="create-button" class="w-100">음악 추가하기</button>
          </router-link>
        </div>
        <div
          class="align-self-start mt-5 w-100"
          :class="{ 'display-none': this.windowWidth < 1300 }"
        >
          <!-- member info -->
          ...
          <div
            v-for="member in content.members"
      ...
      <div
        class="d-flex py-5 flex-column w-100"
        :class="{
          'music-info-1': this.windowWidth > 1300,
          'margin-top': this.windowWidth < 1300,
        }"
      >
        <!-- music info -->
        <div class="mb-5">
          <section class="d-flex flex-column mb-4">
            <span
              class="mb-3"
              style="font-size: 48px; color: #000; font-weight: bold"
            >
              <span v-if="complete"
                >{{ content.band.band_name }}가 완성한 곡</span
              >
              <span v-else>{{ content.band.band_name }}가 진행 중인 곡</span>
            </span>
            <span
              class="px-2"
              style="font-size: 18px; font-weight: 300; color: #888"
            >
          ...
          <ul class="nav nav-tabs">
            <li class="nav-item pr-2">
              <router-link :to="{ name: 'complete' }" class="nav-link">
                <span class="px-3" style="font-size: 18px; font-weight: bold;"
                  >완성한 곡</span
                >
              </router-link>
            </li>
...
export default {
  ...
  computed: {
    ...
    memberOfBand() {
      for (let user_ of this.content.members) {
        if (user_.username === this.user.username) return true;
      }
      return false;
    },
  },
  methods: {
    ...
    leave() {
      if (confirm("'" + this.bandnameParam + "' 밴드에서 나가시겠습니까?"))
        BandService.leaveBand(this.bandnameParam, this.user.username).then(
          this.$store
            .dispatch("bandmusic/getBandInfo", this.bandnameParam)
            .then((response) => {
    ...
    openInvite() {
      this.$vm2.open("modal-invite");
    },
    closeInvite() {
      this.$refs.members.cleanQuery();
      this.$vm2.close("modal-invite");
    },
  },
  mounted() {
    this.$store
